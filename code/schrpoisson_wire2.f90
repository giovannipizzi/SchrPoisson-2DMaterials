...
!This subroutine computes the unscreend potential due to an infinite (along y) charged wire at a distance x

double precision, intent(in) :: x !x is the distance in ang between the wire and the point of interest
double precision, intent(in) :: lambda !lambda is the line charged density of the wire in e/cm

double precision, intent(out) :: potential ! in V
double precision :: lamb ! lambda in units of e/ang
...
! this subroutine computes the potential at a point from a charge density (array of wires with diffrent charges) 

double precision, intent(in) :: coord ! coordinate of point of interest along x axis (in ang)
double precision, intent(in), dimension(n) :: x ! the array of grid points in ang
double precision, intent(in), dimension(n) :: rho_in ! the charge density in e/cm defined at each grid point
double precision, intent(out) :: potential ! the potential in V at point coord due to array of wires
integer, intent(in) :: n ! dimension of all arrays

double precision :: potr !temporary storing of potential on the right (for integration)
double precision :: potl !temporary storing of potential on the left (for integration)
...
! this subroutine computes the potential at each grid point from a charge density (array of wires with diffrent charges) 

double precision, intent(in), dimension(n) :: x ! the array of grid points in ang
double precision, intent(in), dimension(n) :: rho_in ! the charge density in e/cm defined at each grid point
double precision, intent(out), dimension(n) :: potential ! the potential in V at each grid point due to array of wires
integer, intent(in) :: n ! dimension of all arrays

double precision :: coord
double precision :: pot ! for temporar storage of potential
integer i
...
! this subroutine solves the screened Poisson equation for 2D materials with diffrent polarizabilities

double precision, intent(in), dimension(n) :: x ! the array contianing the position of all grid points in ang
double precision, intent(in), dimension(n) :: rho_in ! initial charge density in e/cm defined at each grid point
double precision, intent(in), dimension(n) :: alpha !the value of the polarizability at each grid point in e/V
integer, intent(in) :: max_iteration ! the maximum amount of allowed iteration for the recursive Poisson algorithm
double precision, intent(out), dimension(n) :: potential ! the solution of recursive Poisson in V defined at each grid point
double precision, intent(out), dimension(n) :: rho_out ! The total charge density including polarization charges in e/cm at each grid point
integer, intent(in) :: n ! dimension of all grid-related arrays

!defining all variable needed for the algorithm
integer :: i
integer :: counter, subcounter
integer, dimension(1) :: max_ind
double precision :: beta
double precision :: Vleft
double precision, dimension(2) :: indicator !last 3 variable for fast convergence
...
do while (diff > 1.E-4)
	...
	!one updates the potential including polarization charges
...
!This subroutine computes the unscreend potential due to a periodic array of charged wire at a distance x from one of them

double precision, intent(in) :: x !x is the distance in ang between the wire and the point of interest
double precision, intent(in) :: lambda !lambda is the line charged density of the wire in e/cm
double precision, intent(out) :: potential ! in V
double precision, intent(in) :: period ! periodicity of setup in ang
...
! this subroutine computes the potential at a point from a periodic charge density 
...
double precision :: potr !temporary storing of potential on the right (for integration)
double precision :: potl !temporary storing of potential on the left (for integration)
double precision :: period !the period along the x axis in ang
